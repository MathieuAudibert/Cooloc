...
def statut_candidature(data, token):
    con.connexion()
    ...
    params = (datetime.now(), 'maj statut candidature', id_utilisateur, con.cursor.lastrowid)
    con.cursor.execute(requete3, params)
    
    con.conn.commit()
    con.conn.close()
    return {'status': 200, 'message': 'Statut Candidature modifée avec succès'}